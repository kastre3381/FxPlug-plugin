...
[[fragment]]
float4 fragmentPixelationShader(RasterizerData in [[stage_in]],
                               texture2d<half> colorTexture [[ texture(TI_PixelationInputImage) ]],
                                constant int* size [[ buffer(FIP_Radius)]],
                                constant float* width [[ buffer(FIP_Width)]],
                                constant float* heigth [[ buffer(FIP_Height)]],
                                constant float* texelSizeX [[ buffer(FIP_TexelSizeX)]],
                                constant float* texelSizeY [[ buffer(FIP_TexelSizeY)]])
{
    ...
    if(*size == 0) return static_cast<float4>(colorTexture.sample(textureSampler, in.textureCoordinate));
