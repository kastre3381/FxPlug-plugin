...
/// Function that gets if point is nearby sun
float getSun(float2 vec) {
    return length(vec) < 0.009 ? 1.0 : 0.0;
}

/// Function generating lens flare
float3 lensFlare(float2 textureCoords, float2 sunPos) {
    ...
    /// Calculate angle and distance from sun
...
[[fragment]]
float4 fragmentLensFlareShader(RasterizerData in [[stage_in]], texture2d<half> colorTexture [[texture(TI_LensFlareInputImage)]],
                               constant float2 *location [[buffer(FILF_Location)]], constant float2 *resolution [[buffer(FILF_Resolution)]],
                               constant float3 *sunColor [[buffer(FILF_SunColor)]], constant bool *showImage [[buffer(FILF_ShowImage)]],
    ...
    /// Recalculating position of sun and coordinates of pixel
    ...
    /// Transforming color by making it a little bit darker and using gamma correction
    col = 1.0 - exp(-1.0 * col);
    col = pow(col, float3(1.0 / 2.2));

    /// If frale blending with image is turned off return flare
    if (!*showImage)
        return float4(col * 0.8, 1.0);

    float4 colorSample = static_cast<float4>(colorTexture.sample(textureSampler, in.textureCoordinate));

    /// Calculating the differences between image color sample and flare color
